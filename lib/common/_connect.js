  ...
  return function storageConnect () {
    ...
    let mongo = q.ninvoke(MongoClient, 'connect', uri, Object.assign({ promiseLibrary: Promise }, config.mongo))
      .then(db => {
        ...
        return Promise.all([
          exports.db.users.count(),
          exports.env.get('importing')
        ]).then(([count, importing]) => {
          if (!count && !importing && process.env.DB_PATH) {
            return Promise.all([
              exports.env.set('importing', 1),
              importDB(config)
            ]).then(() => exports.env.del('importing'))
          }
        })
      })
