    ...
    const mongo = q.ninvoke(MongoClient, 'connect', uri, Object.assign({ promiseLibrary: Promise, useUnifiedTopology: true }, config.mongo))
      ...
      .then(async (db) => {
        ...
        if (process.argv[1].endsWith('engine/dist/main.js')) {
          ...
          exports.env.set(exports.env.keys.DATABASE_READY, true)
          return Promise.resolve()
        } else {
          console.log('Waiting for database…')
          return new Promise((resolve) => {
            const awaiter = () => {
              const val = exports.env.get(exports.env.keys.DATABASE_READY)
              if (val) {
                return resolve()
              }
...
exports.importDB = async function importDB (path = './db.json') {
    ...
    if (name === 'env') {
      return env.flushall().then(() => {
        const p = oldcol.data.map(row => {
          ...
          if (row.databaseVersion) {
            ps.push(env.set(env.keys.DATABASE_VERSION, row.databaseVersion))
          }
  ...
  return exports.importMinimalDB()
}
...
exports.importMinimalDB = async function importMinimalDB () {
  ...
  if (!(await env.get(env.keys.GAMETIME))) {
    ps.push(env.set(env.keys.GAMETIME, 1))
  }
  await Promise.all(ps)
}

function alreadyHasStore (object) {
  return object.store != null || object.storeCapacityResource != null || object.storeCapacity != null
}
...
async function upgradeDB () {
  ...
  if (version < 2) {
    ...
    ps.push(db.users.update({ username: 'Screeps' }, { username: 'Screeps', usernameLower: 'screeps', gcl: 0, cpu: 0, active: false, cpuAvailable: 0, badge: { type: 12, color1: '#999999', color2: '#999999', color3: '#999999', flip: false, param: 26 } }, { upsert: true }))
    await Promise.all(ps)
  }
  if (version < 3.1) {
  ...
  if (version < 4) { // Factories update
    ...
    for (const room of busRooms) {
      ...
      if (match && room.depositType == null) {
        ps.push(db.rooms.update({ _id: room._id }, { $set: { depositType: depositTypes[(longitude + latitude) % 4] } }))
      }
    }
    await Promise.all(ps)
  }
  ...
  if (version < 5) {
    ...
    if (powerCreepsCollection) {
      ...
      powerCreeps.forEach(powerCreep => {
        if (alreadyHasStore(powerCreep)) return
    ...
    roomObjects.forEach(object => {
      if (alreadyHasStore(object)) return
